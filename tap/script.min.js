...
a.status="stop";a.interval=false;if(a.user.cont.num>0&&a.level>0&&typeof a.getLevelData()!=="undefined"){document.getElementById("endgame-container").innerHTML='<center> <p id="continue-timer" class="num-timer" >30</p><br> <span class="try-left">还剩'+a.user.cont.num+'次机会 </span><br><center><a class="arcade-mode" href="#" id="continue">接币</a></center><center><a class="survivor-mode" href="#" id="surrender">不了，我妈叫我吃饭</a>  </center></center>';
...
setCookie(this.mode+"_level_record",this.getLevelData().level,365);document.getElementById("endgame-container").innerHTML='<center><p class="new-record">最高记录!! </p><br> <span class="thank">Thank you '+this.user.name+' for playing </span><br> <span class="following">复制以下内容，粘贴分享给你的朋友吧:</span><br><textarea class="text-fol" cols="50" rows="5">我在：'+window.location+" 戳泡泡时，在 "+this.mode.charAt(0).toUpperCase()+this.mode.substring(1)+" 模式下，获得了第 "+this.levelLabel()+" 级，"+this.user.points+" 分的好成绩，来P个K？ </textarea><br><a class=\"arcade-mode\" href=\"#\" id=\"re-start-game\">再玩亿把</a></center><center class='delete-rec' ><a href='#' id='delete-record'>重置分数</a></center>";
}else{document.getElementById("endgame-container").innerHTML='<center><br><br><span class="thank">Thank you '+this.user.name+' for playing </span><br> <span class="following">复制以下内容，粘贴分享给你的朋友吧:</span> <br><textarea class="text-fol" cols="50" rows="5">我玩戳泡泡中， '+window.location+" 在 "+this.mode.charAt(0).toUpperCase()+this.mode.substring(1)+" 模式下，获得了第 "+this.levelLabel()+" 级，"+this.user.points+" 分的好成绩，来P个K？ </textarea><br><a class=\"arcade-mode\" href=\"#\" id=\"re-start-game\">再玩亿把</a></center><center class='delete-rec' ><a href='#' id='delete-record'>重置分数</a></center>";
